...
A guard can be specified by providing the when clause after the arguments list. This is best illustrated by example. The following code tests whether a given number is posi-
...
What gives? The explanation lies in the fact that Elixir terms can be compared with the operators `<` d `>`, even if they’re not of the same type. In this case, the type ordering determines the result:
